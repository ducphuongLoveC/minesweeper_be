...
function pvp(io, socket) {
    ...
    // function handleGameOver(roomId, result, currentGamePlayer, playerState) {
    //     const room = rooms.get(roomId);
    //     if (!room) return;
    ...
    //     const winnerName = getPlayerStatus(room, winner)?.playerName;
    //     const loserName = getPlayerStatus(room, socket.id)?.playerName;
